...
export function ListRdsMajorEol() {
    ...
    const columns = [
        ...
        { field: 'rds_ma_rds_seol', header: 'Standard EOL Date', sortable: 'sortable', visible: 'y' },
        { field: 'ma_row_created_at', header: 'Last AWS refresh', sortable: 'sortable', visible: 'n' },
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                setLoading(true);
                const response = await RdsApi.getRdsEolMajorList();
                console.log(response.data)
                const formattedData = transformRows(response.data.major_eol_data, DATE_COLUMNS);
                setRdsData(formattedData);
                const refreshData = transformRows(response.data.major_refresh_date, REFRESH_COLUMNS);
                setLastRData(refreshData)
                setError(null);                 
            } catch (err) {
                console.error(err);
                setError("Failed to load instances for Major End of life.");
            } finally {
                setLoading(false);
            }
        }
