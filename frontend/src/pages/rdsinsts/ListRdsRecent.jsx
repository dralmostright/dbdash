...
export function ListRdsRecent() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                setLoading(true);
                const response = await RdsApi.getRdsList('recent');
                setRdsData(response.data);
                setError(null);
            } catch (err) {
                console.error(err);
                setError("Failed to load Recently created instances.");
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <>
            <title> DbDash - Recent Instances</title>
            <Header />
            <Sidebar />
